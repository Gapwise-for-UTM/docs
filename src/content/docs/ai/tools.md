...
This catalog is checked against a synchronized copy of AI's [machine-readable live-surface manifest](https://github.com/Gapwise-for-UTM/ai/blob/main/contracts/mcp-live-surface.json). The `gapwise-ai` runtime remains authoritative for schemas returned by MCP discovery.
...
For machine-readable argument and output schemas, use the schemas returned by MCP tool discovery from the [canonical service](/ai/connect/). For deeper implementation detail, see the [`gapwise-ai` tool contract](https://github.com/Gapwise-for-UTM/ai/blob/main/docs/TOOL_CONTRACT.md).
